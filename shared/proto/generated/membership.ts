...
export namespace UserMembershipProductDTO {
  export function fromObject(obj: any): UserMembershipProductDTO {
    return {
      ...obj,
      identifier: UserMembershipProductDTO.ProductIdentifier_fromString(obj.identifier),
      benefitPackageType: UserMembershipProductDTO.BenefitPackageType_fromString(obj.benefitPackageType),
    }
  }
   ...
   * GIFT : 
     *  기프트 패키지 (2회차 이상 혜택)
   */
  export type BenefitPackageType = 'UNKNOWN' | 'WELCOME' | 'GIFT';

  export function BenefitPackageType_fromString(str: string): BenefitPackageType | undefined {
    switch (str) {
      case "UNKNOWN":
      case "WELCOME":
      case "GIFT":
...
export namespace UserMembershipDTO {
  export function fromObject(obj: any): UserMembershipDTO {
    return {
      ...obj,
      productIdentifier: UserMembershipProductDTO.ProductIdentifier_fromString(obj.productIdentifier),
      source: UserMembershipDTO.UserMembershipSource_fromString(obj.source),
      paymentMethod: obj.paymentMethod != null ? PaymentMethodDTO.fromObject(obj.paymentMethod) : undefined,
      userSubscriptionStatus: UserSubscriptionDTO.Status_fromString(obj.userSubscriptionStatus),
      ...
      benefitPackageType: UserMembershipProductDTO.BenefitPackageType_fromString(obj.benefitPackageType),
      refundAvailable: obj.refundAvailable != null ? BoolValue.fromObject(obj.refundAvailable) : undefined,
    }
  }

  export type UserMembershipSource = 'UNKNOWN' | 'VCNC' | 'SOCAR';

  export function UserMembershipSource_fromString(str: string): UserMembershipSource | undefined {
    switch (str) {
      case "UNKNOWN":
      case "VCNC":
      case "SOCAR":
