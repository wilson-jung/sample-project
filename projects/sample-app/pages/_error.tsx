...
const captureExceptionAndFlush = async (err: string | Error, flushAfter = 2000) => {
  if (SENTRY_DSN) {
    sentryCaptureException(err)
    if (flushAfter > 0) {
...
CustomError.getInitialProps = async ({ res, err, asPath }: AugmentedNextPageContext) => {
  ...
  // Flushing before returning is necessary if deploying to Vercel, see
  // https://vercel.com/docs/platform/limits#streaming-responses
  await captureExceptionAndFlush(new Error(`_error.js getInitialProps missing data at path: ${asPath}`), 2000)
